...
Host data mounts default to `./data` on both platforms. Network binding defaults to `COMFYUI_BIND_HOST=0.0.0.0` so internal network clients can connect directly; set it to `127.0.0.1` for SSH-tunnel-only operation. Linux installation dynamically writes UID/GID values into `.env`; Windows installation leaves UID/GID blank and relies on Compose defaults.
